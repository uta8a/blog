...
メモリが少ないからです。
Linuxをどう定義するかにもよるのですが、無理なくLinuxを動作させるためには最低でも数MBは必要で、それに対してArduinoの例えばArm Cortex-M4のRAMは数百KBくらいしかないし、外部RAMも搭載していません。また、CortexMシリーズだとMMUが無いので仮想記憶が作れないことがLinuxが動かせないメインの理由のようです。
...
マストドンを建てる手順通りにやるとうまく行くらしいです。
一方で、Developmentの方は公式ドキュメントに記載がないらしく、production用のドキュメントを参考にする必要があるため少し大変とのことでした。

マストドンは日本語での検索にデフォルトで対応していないので、日本語の形態素解析を行うようにパッチを当てる必要があります。ElasticsearchのプラグインとしてKuromojiを使用するよう変更を当てている話がされました。(そこからElasticsearchの話につながっていった)
